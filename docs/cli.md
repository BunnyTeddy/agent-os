...
when added with `channels add slack`. The default is `/slack/events`; an empty
field uses the same URL. Adding or enabling another account preserves existing
paths. Set a distinct path on each additional webhook account, for example with
`agentos channels edit team-b --field webhook_path=/slack/events/team-b`, and
configure that Slack app's Request URLs to match. Duplicate webhook paths with
overlapping HTTP methods are rejected at gateway startup with an error naming
the entries and field to change. Disabled and Socket Mode entries do not register
webhook routes. See [Slack modes](channels.md#slack-modes).
