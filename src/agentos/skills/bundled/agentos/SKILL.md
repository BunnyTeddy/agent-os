...
Slack webhook entries use `webhook_path` (default `/slack/events`; empty also
uses this URL). Adding or enabling another account preserves existing URLs.
Configure a distinct path for each additional webhook account with
`channels add slack --field webhook_path=…` or
`channels edit <name> --field webhook_path=…`; for example, a second account can
use `/slack/events/team-b` while the first keeps `/slack/events`. Configure each
Slack app's Events API, Interactivity, and slash-command Request URLs to match,
then restart the gateway. Duplicate webhook paths with overlapping HTTP methods
cause a startup error naming the entries and field to change. Disabled and
Socket Mode entries do not register webhook routes.
